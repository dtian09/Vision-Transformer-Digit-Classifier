...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        B, C, H, W = x.shape
        x = x.unfold(2, patch_size, patch_size).unfold(3, patch_size, patch_size)  # B x C x 4 x 4 x 7 x 7
        x = x.contiguous().view(B, C, -1, patch_size, patch_size)  # B x C x 16 x 7 x 7
        x = x.unfold(2, 7, 7).unfold(3, 7, 7)  # B x C x 4 x 4 x 7 x 7
        x = x.contiguous().view(B, C, -1, 7, 7)  # B x C x 16 x 7 x 7
        x = x.permute(0, 2, 3, 4, 1).reshape(B, 16, -1)  # B x 16 x 49
        return self.proj(x)  # B x 16 x emb_dim
